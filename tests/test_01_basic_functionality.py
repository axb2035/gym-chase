...
def test_chase_environment(env: gym.Env) -> None:
    ...
    9. Run 20,000 episodes where the no move action is selected to test win condition.
    ...
    ), "Observation space is not a dictionary."
    assert set(env.observation_space.spaces.keys()) == {
        "agent",
        "robots",
        "zappers",
    }, "Observation space has incorrect keys."
    ...
    assert set(robots_space.spaces.keys()) == set(
        range(env.robots)
    ), "Robots space has incorrect keys."
    for key, value in robots_space.spaces.items():
        assert isinstance(value, Dict), "Robot space is not a Dict space."
        assert set(value.spaces.keys()) == {
            "alive",
            "location",
        }, "Robot space has incorrect keys."
        assert isinstance(
            value.spaces["alive"], Discrete
        ), "Alive space is not a Discrete space."
        assert (
            value.spaces["alive"].n == 2
        ), "Alive space has incorrect number of values."
        assert isinstance(
            value.spaces["location"], Box
        ), "Location space is not a Box space."
        assert value.spaces["location"].shape == (
            2,
        ), "Location space has incorrect shape."
        assert (
            value.spaces["location"].dtype == np.int32
        ), "Location space has incorrect dtype."
    ...
    assert set(zappers_space.spaces.keys()) == set(
        range(env.zappers)
    ), "Zappers space has incorrect keys."
    ...
    for e in tqdm(range(1000), desc="Random actions"):
        terminated = False
        total_reward = 0
        ...
        while not terminated:
            ...
            assert isinstance(
                terminated, bool
            ), "Terminated flag is not a boolean value."
    ...
    for e in tqdm(range(1000), desc="Possum actions"):
        terminated = False
        total_reward = 0
        state, info = env.reset(seed=e)
        while not terminated:
